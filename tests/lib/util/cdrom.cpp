...
static void cdm1_test_put_u16be(
...
static void cdm1_test_put_u32be(
		std::vector<uint8_t> &data,
		size_t offset,
		uint32_t value)
{
...
static std::vector<uint8_t> make_valid_cdm1_test_metadata()
{
	...
	constexpr std::array<cdrom_file::cdm1_section, section_count> sections =
	{
	...
	constexpr std::array<uint32_t, section_count> record_sizes =
	{
	...
	for (uint32_t i = 0; i < section_count; i++)
		total_bytes +=
				cdrom_file::CDM1_TABLE_HEADER_BYTES
					+ record_counts[i] * record_sizes[i];

	std::vector<uint8_t> metadata(total_bytes, 0);

	cdm1_test_put_u32be(metadata, 0, cdrom_file::CDM1_MAGIC);
	cdm1_test_put_u16be(metadata, 4, cdrom_file::CDM1_VERSION_MAJOR);
	cdm1_test_put_u16be(metadata, 6, cdrom_file::CDM1_VERSION_MINOR);
	cdm1_test_put_u32be(metadata, 8, cdrom_file::CDM1_HEADER_BYTES);
	cdm1_test_put_u32be(metadata, 12, total_bytes);
	cdm1_test_put_u32be(metadata, 16, section_count);
	cdm1_test_put_u32be(metadata, 20, directory_offset);

	uint32_t section_offset = first_section_offset;
	...
	for (uint32_t i = 0; i < section_count; i++)
	{
		...
		cdm1_test_put_u32be(metadata, entry + 0, uint32_t(sections[i]));
		cdm1_test_put_u16be(metadata, entry + 4, 1);
		cdm1_test_put_u16be(
				metadata,
				entry + 6,
				uint16_t(cdrom_file::cdm1_section_flag::required));
		cdm1_test_put_u32be(metadata, entry + 8, section_offset);
		cdm1_test_put_u32be(metadata, entry + 12, section_length);
		cdm1_test_put_u32be(metadata, entry + 16, record_counts[i]);
		cdm1_test_put_u32be(metadata, entry + 20, 0);
		...
		cdm1_test_put_u32be(
				...
				section_offset + 4,
				record_counts[i]);
		cdm1_test_put_u32be(metadata, section_offset + 8, 0);
		cdm1_test_put_u32be(metadata, section_offset + 12, 0);
...
TEST_CASE("CD-ROM CDM1 metadata structure validation", "[util][cdrom]")
{
	...
	{
		const std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u32be(metadata, 12, uint32_t(metadata.size() - 1));
	...
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u32be(metadata, 24, 1);
	...
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u32be(metadata, 28, 1);
	...
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u32be(metadata, 16, 7);
	...
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u32be(metadata, 20, cdrom_file::CDM1_HEADER_BYTES + 1);
	...
	SECTION("sections overlap")
	{
		...
		cdm1_test_put_u32be(metadata, second_entry + 8, first_offset);
		...
		SECTION("section shorter than table header")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(
				metadata,
				cdrom_file::CDM1_HEADER_BYTES + 12,
				cdrom_file::CDM1_TABLE_HEADER_BYTES - 1);
	...
	SECTION("wrong table record size")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(
				metadata,
				first_section_offset,
				cdrom_file::CDM1_DISC_RECORD_BYTES + 1);
	...
	SECTION("table record count disagrees with directory")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("nonzero table flags")
	{
		...
		cdm1_test_put_u32be(metadata, first_section_offset + 8, 1);
	...
	SECTION("nonzero table reserved field")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, first_section_offset + 12, 1);
	...
	SECTION("table length does not match records")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("DISC flags must be zero")
	{
		...
		SECTION("TRAK id must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, track_record_offset + 0, 0);
	...
	SECTION("TRAK ids must be unique")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(
				metadata,
				track_record_offset + cdrom_file::CDM1_TRACK_RECORD_BYTES,
				1);
	...
	SECTION("TRAK session id must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, track_record_offset + 4, 0);
	...
	SECTION("TRAK number must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, track_record_offset + 8, 0);
	...
	SECTION("TRAK numbers must be unique within a session")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("TRAK type must be recognized")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, track_record_offset + 10, 0xffff);
	...
	SECTION("TRAK flags must be zero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, track_record_offset + 14, 1);
	...
	SECTION("REGN length may be unknown")
	{
		...
		SECTION("EVID id must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();

		const uint32_t evidence_record_offset =
				cdrom_file::CDM1_HEADER_BYTES
					+ 8 * cdrom_file::CDM1_SECTION_ENTRY_BYTES
					+ cdrom_file::CDM1_TABLE_HEADER_BYTES
					+ cdrom_file::CDM1_DISC_RECORD_BYTES
		...
		cdm1_test_put_u32be(metadata, evidence_record_offset + 0, 0);
	...
	SECTION("EVID ids must be unique")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("EVID flags must be zero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, evidence_record_offset + 4, 1);
	...
	SECTION("EVID evidence class must be recognized")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, evidence_record_offset + 8, 0);
		...
		metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u16be(metadata, evidence_record_offset + 8, 10);
	...
	SECTION("EVID coordinate class must be recognized")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, evidence_record_offset + 10, 0);
		...
		metadata = make_valid_cdm1_test_metadata();
		cdm1_test_put_u16be(metadata, evidence_record_offset + 10, 8);
		...
		SECTION("STOR id must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();

		const uint32_t storage_record_offset =
				cdrom_file::CDM1_HEADER_BYTES
					+ 8 * cdrom_file::CDM1_SECTION_ENTRY_BYTES
					+ cdrom_file::CDM1_TABLE_HEADER_BYTES
					+ cdrom_file::CDM1_DISC_RECORD_BYTES
		...
		cdm1_test_put_u32be(metadata, storage_record_offset + 0, 0);
	...
	SECTION("STOR ids must be unique")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("STOR flags must be zero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, storage_record_offset + 4, 1);
	...
	SECTION("STOR class must be recognized")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, storage_record_offset + 8, 0);
	...
	SECTION("STOR encoding must be recognized")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u16be(metadata, storage_record_offset + 10, 1);
	...
	SECTION("STOR unit size must be nonzero")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, storage_record_offset + 12, 0);
	...
	SECTION("MAPS region source reference must resolve")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, mapping_record_offset + 4, 999);
	...
	SECTION("MAPS evidence target reference must resolve")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u32be(metadata, mapping_record_offset + 8, 999);
	...
	SECTION("MAPS evidence source reference must resolve")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		const uint32_t mapping_record_offset =
				cdrom_file::CDM1_HEADER_BYTES
					...
					+ 2 * cdrom_file::CDM1_STORAGE_RECORD_BYTES
					+ cdrom_file::CDM1_TABLE_HEADER_BYTES
					+ 4 * cdrom_file::CDM1_MAPPING_RECORD_BYTES;
	...
	SECTION("MAPS storage target reference must resolve")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("MAPS region span must fit known region")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u64be(metadata, mapping_record_offset + 24, 1);
	...
	SECTION("MAPS evidence span must fit evidence extent")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
	...
	SECTION("MAPS storage span must fit storage extent")
	{
		std::vector<uint8_t> metadata = make_valid_cdm1_test_metadata();
		...
		cdm1_test_put_u64be(metadata, mapping_record_offset + 40, 1);
...
const std::array<cdrom_file::cdm1_section, 8> expected_types =
{
	cdrom_file::cdm1_section::disc,
	cdrom_file::cdm1_section::sessions,
	cdrom_file::cdm1_section::tracks,
...
const std::array<uint32_t, 8> expected_record_sizes =
{
	cdrom_file::CDM1_DISC_RECORD_BYTES,
	cdrom_file::CDM1_SESSION_RECORD_BYTES,
	cdrom_file::CDM1_TRACK_RECORD_BYTES,
	...
	cdrom_file::CDM1_MAPPING_RECORD_BYTES
		...
		uint32_t expected_offset = first_section_offset;

		for (uint32_t i = 0; i < sections.size(); i++)
		{
			const uint32_t expected_count = i + 1;
			const uint32_t expected_length =
					cdrom_file::CDM1_TABLE_HEADER_BYTES
						+ expected_count * expected_record_sizes[i];
			...
			REQUIRE(
					sections[i].flags
						== uint16_t(cdrom_file::cdm1_section_flag::required));
			REQUIRE(sections[i].offset == expected_offset);
			REQUIRE(sections[i].length == expected_length);
			REQUIRE(sections[i].count == expected_count);
