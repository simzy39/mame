...
std::error_condition validate_cdm1_track_records(
		const uint8_t *data,
		uint32_t directory_offset)
{
	const uint8_t *const track_entry =
			data
				+ directory_offset
				+ 2 * cdrom_file::CDM1_SECTION_ENTRY_BYTES;
	...
	for (uint32_t i = 0; i < record_count; i++)
	{
		...
		const uint32_t id = get_u32be(record + 0);
		const uint32_t session_id = get_u32be(record + 4);
		const uint16_t number = get_u16be(record + 8);
		const uint16_t track_type = get_u16be(record + 10);
		const uint16_t control_flags = get_u16be(record + 12);
		const uint16_t flags = get_u16be(record + 14);

		if (id == 0
				|| session_id == 0
				|| number == 0
				|| track_type > uint16_t(cdrom_file::cdm1_track_type::audio)
				|| (control_flags & ~uint16_t(0x000f))
				|| flags != 0)
		{
			return chd_file::error::INVALID_DATA;
		}
		...
		for (uint32_t previous = 0; previous < i; previous++)
		{
			...
			if (get_u32be(previous_record + 0) == id
					|| (get_u32be(previous_record + 4) == session_id
...
std::error_condition validate_cdm1_mapping_records(
		const uint8_t *data,
		uint32_t directory_offset)
{
	const uint8_t *const mapping_entry =
			data
				+ directory_offset
	...
	for (uint32_t i = 0; i < record_count; i++)
	{
		...
		const uint32_t id = get_u32be(record + 0);
		const uint32_t source_id = get_u32be(record + 4);
		...
		const uint16_t mapping_class = get_u16be(record + 20);
		const uint16_t provenance = get_u16be(record + 22);
		...
		const uint64_t target_length = get_u64be(record + 48);
		...
		if (id == 0
				...
				|| provenance
					> uint16_t(cdrom_file::cdm1_provenance::generated)
				|| source_length == 0
				|| target_length == 0
				|| source_offset > UINT64_MAX - source_length
				|| target_offset > UINT64_MAX - target_length)
		{
			return chd_file::error::INVALID_DATA;
		}
...
std::error_condition validate_cdm1_mapping_references(
		const uint8_t *data,
		uint32_t directory_offset)
{
	auto get_records =
	...
	const uint8_t *const regions = get_records(4);
	const uint8_t *const evidence = get_records(5);
	const uint8_t *const storage = get_records(6);
	const uint8_t *const mappings = get_records(7);

	const uint32_t region_count = get_count(4);
	const uint32_t evidence_count = get_count(5);
	const uint32_t storage_count = get_count(6);
	const uint32_t mapping_count = get_count(7);
	...
	auto find_evidence =
			[evidence, evidence_count](uint32_t id) -> const uint8_t *
			{
	...
	auto find_storage =
			[storage, storage_count](uint32_t id) -> const uint8_t *
			{
	...
	auto validate_endpoint =
			[&find_region, &find_evidence, &find_storage](
					uint16_t kind,
					uint32_t id,
					uint64_t offset,
					uint64_t length)
			{
				if (kind
						== uint16_t(cdrom_file::cdm1_mapping_object::region))
				{
					const uint8_t *const record = find_region(id);
					...
					if (!(flags
							& uint16_t(
									cdrom_file::cdm1_region_flag::length_known)))
					{
						return true;
					}

					const uint64_t extent = get_u64be(record + 24);
				...
				{
					const uint8_t *const record = find_evidence(id);

					if (!record)
						return false;

					const uint64_t extent = get_u64be(record + 16);
				...
				if (kind
						== uint16_t(cdrom_file::cdm1_mapping_object::storage))
				{
					const uint8_t *const record = find_storage(id);
		...
		const std::vector<uint8_t> &metadata)
{
	...
	const std::error_condition mapping_err =
			validate_cdm1_mapping_records(
					data,
					directory_offset);
	...
	const std::error_condition mapping_reference_err =
			validate_cdm1_mapping_references(
					data,
					directory_offset);

	if (mapping_reference_err)
		return mapping_reference_err;
